...
Function UpdateTXT()
   ' Don't save if critical fields are blank/invalid - prevents corruption
   Dim s, p
   s = Trim(txt_SchLib.Text)
   p = Trim(txt_PcbLib.Text)

   ' If the path boxes are empty or don't look like library files, skip the save
...
Function AddPcbLib(footprint)
    ...
    Set PCBLib = PCBServer.GetCurrentPCBLibrary
    If PCBLib Is Nothing Then
       MsgBox "No PCB library is focused - the .PcbLib document did not open.", vbSystemModal, "Altium Library Loader"
       AddPcbLib = False
       Exit Function
    End If
